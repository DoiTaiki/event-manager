...
- **目標値**: デフォルト 70%（パラメータ `ECSScalingTargetCpuPercent` で変更可能）
- **タスク数**: 最小・最大タスク数はパラメータで指定（デフォルトは最小 2、最大 4。`ECSMinCapacity` / `ECSMaxCapacity`）
- **クールダウン**: スケールイン・スケールアウトともに 60 秒

負荷に応じてタスク数が自動で増減し、目標 CPU 使用率に近づくように調整されます。パラメータの詳細は「[1. CloudFormation テンプレートの準備](#1-cloudformation-テンプレートの準備)」の「[任意パラメータ](#任意パラメータ)」および「[オプション機能](#オプション機能)」の「[ECS の自動スケーリング](#ecs-の自動スケーリング-1)」を参照してください。
...
- `ECSMinCapacity`: ECS の最小タスク数（自動スケーリング下限、デフォルト: 2、1〜50）。詳細は「[オプション機能](#オプション機能)」の「[ECS の自動スケーリング](#ecs-の自動スケーリング-1)」を参照。
- `ECSMaxCapacity`: ECS の最大タスク数（自動スケーリング上限、デフォルト: 4、1〜50）。
- `ECSScalingTargetCpuPercent`: ECS の目標平均 CPU 使用率（%、デフォルト: 70、1〜100）。スケーリングでこの値に近づくようにタスク数が調整されます。
...
ECS サービスには Application Auto Scaling によるターゲット追跡スケーリングが常時有効です。メトリクスに ECS サービスの平均 CPU 使用率を使用し、目標値に近づくようにタスク数が自動で増減します。

| パラメータ | 説明 | デフォルト |
|-----------|------|-----------|
| `ECSMinCapacity` | 最小タスク数（1〜50） | 2 |
| `ECSMaxCapacity` | 最大タスク数（1〜50） | 4 |
| `ECSScalingTargetCpuPercent` | 目標平均 CPU 使用率（%、1〜100） | 70 |

スケールイン・スケールアウトのクールダウンはそれぞれ 60 秒です。負荷に応じたコストとパフォーマンスのバランスを取る場合は、これらのパラメータをスタック作成時または更新時に指定してください。
