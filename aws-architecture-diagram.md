...
- **ECS サービス**: プライベートサブネットで実行（タスク数は自動スケーリングで調整）
- **ECS 自動スケーリング**: Application Auto Scaling によるターゲット追跡（メトリクス: 平均CPU使用率、デフォルト目標70%、最小/最大タスク数はパラメータで指定、スケールイン/アウト クールダウン 60 秒）
